...
def _make_circle(x0: np.ndarray, r: float) -> np.ndarray:
...
def adjugate(C: np.ndarray) -> np.ndarray:
    det = np.linalg.det(C)
...
def skew_symmetric(C: np.ndarray) -> np.ndarray:
    a, b, c = C
...
def estimate_pose(Q: np.ndarray, r, alpha):
